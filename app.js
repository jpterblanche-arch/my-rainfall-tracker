...
function settings(){return `<div class="panel"><h2>Settings</h2><p class="sub">Rainy day threshold: <b>0.1 mm</b>. Rainfall intensity bands are ready for future use: 0, 0.1–5, 5–20, 20–50, and 50+ mm.</p><p class="sub">This local edition stores records in this browser. A PostgreSQL-backed deployment can use the same fields: id, rainfall_date, rainfall_mm, notes, created_at, and updated_at with a unique rainfall_date constraint.</p></div>`}
...
function bind(){if(page==='record'){$('#record-form').onsubmit=e=>{e.preventDefault();const v=Object.fromEntries(new FormData(e.target)), rows=read(), n=Number(v.rainfall_mm);if(!v.date||!Number.isFinite(n)||n<0)return message('Rainfall must be a valid value of 0 mm or more.',true);if(rows.some(r=>r.date===v.date)&&(!editing||editing.date!==v.date))return message('A rainfall record already exists for this date.',true);if(editing)rows.splice(rows.findIndex(r=>r.date===editing.date),1);
