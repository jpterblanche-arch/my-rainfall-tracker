...
function chart(values, labels){const max=Math.max(...values,1);return `<div class="chart">${values.map((v,i)=>`<div class="bar" style="height:${Math.max(v?6:1,v/max*100)}%" data-tip="${labels[i]}: ${money(v)}" tabindex="0"></div>`).join('')}</div><div class="sub chart-labels" style="display:flex;justify-content:space-between;margin-top:8px">${labels.map(label=>`<span>${label}</span>`).join('')}</div>`}
